//! Text chat workflows for `Wagmii` and Wagmii-compatible APIs.
...
pub async fn run_wagmii_chat(client: &WagmiiClient, options: TextChatOptions) -> Result<()> {
    let mut messages: Vec<Message> = Vec::new();
    let mut stats = SessionStats::new();

    print_banner("Wagmii Compatible API");
    print_session_info(
        &options,
        messages.len(),
    ...
    if let Some(prompt) = options.prompt.as_deref() {
        process_wagmii_turn(client, &options, &mut messages, prompt, &mut stats).await?;
    } else {
        let mut rl = create_editor()?;
        while let Some(line) = read_prompt(&mut rl)? {
            if handle_line_wagmii(line, client, &options, &mut messages, &mut stats).await? {
                break;
            }
        }
    }
...
pub async fn run_official_chat(client: &WagmiiClient, options: TextChatOptions) -> Result<()> {
...
fn print_banner(mode: &str) {
    println!("{}", ds_blue("Wagmii CLI").bold());
    println!("Mode: {mode}");
...
fn ds_blue(text: &str) -> ColoredString {
    let (r, g, b) = palette::WAGMII_BLUE_RGB;
    text.truecolor(r, g, b)
}

fn ds_sky(text: &str) -> ColoredString {
    let (r, g, b) = palette::WAGMII_SKY_RGB;
    text.truecolor(r, g, b)
}

fn ds_red(text: &str) -> ColoredString {
    let (r, g, b) = palette::WAGMII_RED_RGB;
    text.truecolor(r, g, b)
}
...
fn history_path() -> Option<std::path::PathBuf> {
    dirs::home_dir().map(|home| {
        let dir = home.join(".wagmii");
        let _ = std::fs::create_dir_all(&dir);
        dir.join("history")
    })
}

async fn handle_line_wagmii(
    line: String,
    client: &WagmiiClient,
    options: &TextChatOptions,
    messages: &mut Vec<Message>,
    ...
    if handle_command_wagmii(input, messages, Some(options), stats) {
        return Ok(false);
    }
    if let Err(error) = process_wagmii_turn(client, options, messages, input, stats).await {
