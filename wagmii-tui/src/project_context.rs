//! Project context loading for wagmii-cli.
//!
...
//! - `.wagmii/instructions.md` - Hidden instructions file (legacy)
//!
...
fn check_trust_status(workspace: &Path) -> bool {
    ...
    let trust_markers = [
        workspace.join(".wagmii").join("trusted"),
        workspace.join(".wagmii").join("trust.json"),
    ];
...
This file provides guidance to AI agents (Wagmii CLI, Claude Code, etc.) when working with code in this repository.
...
mod tests {
    ...
    fn test_load_project_context_hidden_dir() {
        let tmp = tempdir().expect("tempdir");
        let hidden_dir = tmp.path().join(".wagmii");
    ...
    fn test_check_trust_status() {
        ...
        // Create trust marker
        let wagmii_dir = tmp.path().join(".wagmii");
        fs::create_dir(&wagmii_dir).expect("mkdir");
        fs::write(wagmii_dir.join("trusted"), "").expect("write");
