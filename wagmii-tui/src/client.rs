//! HTTP client for the Wagmii OpenAI-compatible APIs.
//!
...
/// Client for Wagmii's OpenAI-compatible APIs.
#[must_use]
pub struct WagmiiClient {
...
const ALLOW_INSECURE_HTTP_ENV: &str = "WAGMII_ALLOW_INSECURE_HTTP";
...
impl TokenBucket {
    fn from_env() -> Self {
        let rps = std::env::var("WAGMII_RATE_LIMIT_RPS")
            .ok()
            .and_then(|v| v.parse::<f64>().ok())
            .unwrap_or(DEFAULT_CLIENT_RATE_LIMIT_RPS)
            .max(0.0);
        let burst = std::env::var("WAGMII_RATE_LIMIT_BURST")
            .ok()
            .and_then(|v| v.parse::<f64>().ok())
...
impl WagmiiClient {
    /// Create a Wagmii client from CLI configuration.
    pub fn new(config: &Config) -> Result<Self> {
        let api_key = config.wagmii_api_key()?;
        let base_url = config.wagmii_base_url();
        ...
        logging::info(format!("Wagmii base URL: {base_url}"));
        logging::info(format!(
    ...
    async fn create_message_responses(
        ...
        if !status.is_success() {
            let error_text = bounded_error_text(response, ERROR_BODY_MAX_BYTES).await;
            anyhow::bail!("Failed to call Wagmii Responses API: HTTP {status}: {error_text}");
        }
    ...
    async fn create_message_chat(&self, request: &MessageRequest) -> Result<MessageResponse> {
        ...
        if !status.is_success() {
            let error_text = bounded_error_text(response, ERROR_BODY_MAX_BYTES).await;
            anyhow::bail!("Failed to call Wagmii Chat API: HTTP {status}: {error_text}");
        }
...
impl LlmClient for WagmiiClient {
...
fn build_chat_messages(
    ...
    for message in messages {
        ...
        if role == "assistant" {
            ...
            // Wagmii rejects assistant messages where both `content` and
    ...
    while i < out.len() {
        ...
        if is_assistant_with_tools {
            ...
            if !expected_ids.is_subset(&found_ids) {
                ...
                // assistant message entirely (Wagmii requires content or tool_calls).
...
fn requires_reasoning_content(model: &str) -> bool {
    ...
    lower.contains("wagmii-v3.2")
        || lower.contains("reasoner")
        || lower.contains("-reasoning")
        || lower.contains("-thinking")
        || has_wagmii_r_series_marker(&lower)
}

fn has_wagmii_r_series_marker(model_lower: &str) -> bool {
    const PREFIX: &str = "wagmii-r";
...
fn parse_sse_chunk(
    ...
    for choice in choices {
        ...
        if let Some(delta) = delta {
            // Handle reasoning_content (Wagmii-Reasoner thinking)
...
mod tests {
    ...
    fn chat_messages_include_reasoning_content_for_reasoner() {
        ...
        let out = build_chat_messages(None, &[message], "wagmii-reasoner");
        let assistant = out
            .iter()
    ...
    fn chat_messages_drop_thinking_only_assistant_for_chat_model() {
        ...
        let out = build_chat_messages(None, &[message], "wagmii-chat");
        assert!(
            !out.iter()
    ...
    fn chat_messages_drop_thinking_only_assistant_for_r_series_model() {
        ...
        let out = build_chat_messages(None, &[message], "wagmii-r2-lite-preview");
        assert!(
            !out.iter()
                .any(|value| value.get("role").and_then(Value::as_str) == Some("assistant"))
        );
    ...
    fn chat_messages_drop_thinking_only_assistant_for_v4_mini_model() {
        ...
        let out = build_chat_messages(None, &[message], "wagmii-v4-mini");
        assert!(
            !out.iter()
                .any(|value| value.get("role").and_then(Value::as_str) == Some("assistant"))
        );
    ...
    fn chat_messages_drop_orphan_tool_results() {
        ...
        let out = build_chat_messages(None, &messages, "wagmii-chat");
        assert!(
            !out.iter()
    ...
    fn parse_models_response_parses_and_deduplicates() {
        let payload = r#"{
            "object": "list",
            "data": [
                {"id": "wagmii-r1", "object": "model", "owned_by": "wagmii", "created": 1},
                {"id": "wagmii-chat", "object": "model"},
                {"id": "wagmii-r1", "object": "model", "owned_by": "wagmii", "created": 1}
            ]
        }"#;
        ...
        assert_eq!(
            ...
            vec![
                AvailableModel {
                    id: "wagmii-chat".to_string(),
                    owned_by: None,
                    created: None
                },
                AvailableModel {
                    id: "wagmii-r1".to_string(),
                    owned_by: Some("wagmii".to_string()),
                    created: Some(1)
                }
            ]
        );
    ...
    fn base_url_security_rejects_insecure_non_local_http() {
        let err = validate_base_url_security("http://api.wagmii.com")
