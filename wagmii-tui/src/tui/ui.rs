//! TUI event loop and rendering logic for `Wagmii` CLI.
...
const UI_WAGMII_SQUIGGLE_MS: u64 = 120;
const UI_TYPING_INDICATOR_MS: u64 = 120;
const UI_STATUS_ANIMATION_MS: u64 = UI_WAGMII_SQUIGGLE_MS;
            ...
            while let Ok(event) = rx.try_recv() {
                ...
                match event {
                    ...
                    EngineEvent::MessageComplete { .. } => {
                        ...
                        // Wagmii rejects assistant messages that contain only reasoning blocks.
...
fn render_sidebar_tasks(f: &mut Frame, area: Rect, app: &App) {
    ...
    if let Some(turn_id) = app.runtime_turn_id.as_ref() {
        ...
        lines.push(Line::from(Span::styled(
            ...
            Style::default().fg(palette::WAGMII_SKY),
        )));
    }
    ...
    if app.task_panel.is_empty() {
        ...
        lines.push(Line::from(vec![
            Span::styled(
                ...
                Style::default().fg(palette::WAGMII_SKY).bold(),
            ),
            Span::styled(
    ...
    let section = Paragraph::new(lines).wrap(Wrap { trim: false }).block(
        Block::default()
            .title(Line::from(vec![Span::styled(
                format!(" {title} "),
                Style::default().fg(palette::WAGMII_BLUE).bold(),
            )]))
            .borders(Borders::ALL)
            .border_style(Style::default().fg(palette::BORDER_COLOR))
            .style(Style::default().bg(palette::WAGMII_INK))
...
fn render_status_indicator(
    ...
    if app.is_loading {
        ...
        let spinner = if has_streaming_content {
            ...
            wagmii_squiggle(app.turn_started_at)
        };
        let (label, label_color) = if has_streaming_content {
            ("ANSWER", palette::WAGMII_SKY)
        ...
        let mut spans = vec![
            Span::styled(spinner, Style::default().fg(palette::WAGMII_SKY).bold()),
            Span::raw(" "),
    ...
    if let Some(draft) = app.queued_draft.as_ref() {
        ...
        lines.push(Line::from(vec![
            ...
            Span::raw(" "),
            Span::styled(preview, Style::default().fg(palette::WAGMII_SKY)),
        ]));
    }
...
fn status_color(level: StatusToastLevel) -> ratatui::style::Color {
    match level {
        StatusToastLevel::Info => palette::WAGMII_SKY,
...
fn wagmii_squiggle(start: Option<Instant>) -> &'static str {
    const FRAMES: [&str; 6] = ["◍", "◉", "◌", "◌", "◉", "◍"];
    let elapsed_ms = start.map_or(0, |t| t.elapsed().as_millis());
    let idx = ((elapsed_ms / u128::from(UI_WAGMII_SQUIGGLE_MS)) as usize) % FRAMES.len();
    FRAMES[idx]
}
