...
use crate::{commands, config::COMMON_WAGMII_MODELS};
use ratatui::{
    buffer::Buffer,
    layout::Rect,
    prelude::Stylize,
    ...
    fn render(&self, area: Rect, buf: &mut Buffer) {
        ...
        if self.app.input.is_empty() {
            ...
            for (idx, line) in visible_lines.iter().enumerate() {
                ...
                lines.push(Line::from(vec![
                    Span::styled(prefix, Style::default().fg(palette::WAGMII_SKY).bold()),
...
mod tests {
    ...
    #[test]
    fn slash_completion_hints_exclude_set_and_wagmii_commands() {
        let hints = slash_completion_hints("/", 128);
        assert!(!hints.iter().any(|hint| hint == "/set"));
        assert!(!hints.iter().any(|hint| hint == "/wagmii"));
    }
