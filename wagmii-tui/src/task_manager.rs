//! Persistent background task manager for Wagmii agent work.
//!
//! Tasks are durable across restarts and execute with a bounded worker pool.
//! Execution stays Wagmii-only and now links every task to runtime
...
/// Engine-backed executor (Wagmii-only).
...
impl TaskManager {
    /// Start the manager with the default Wagmii executor.
...
/// Default task persistence location (`~/.wagmii/tasks`).
#[must_use]
pub fn default_tasks_dir() -> PathBuf {
    if let Ok(path) = std::env::var("WAGMII_TASKS_DIR")
        && !path.trim().is_empty()
    {
        return PathBuf::from(path);
    }
    if let Some(home) = dirs::home_dir() {
        return home.join(".wagmii").join("tasks");
    }
    PathBuf::from(".wagmii").join("tasks")
}
...
mod tests {
    ...
    fn test_config(root: PathBuf) -> TaskManagerConfig {
        TaskManagerConfig {
            ...
            default_model: "wagmii-v3.2".to_string(),
            default_mode: "agent".to_string(),
            allow_shell: false,
            trust_mode: false,
            max_subagents: 2,
    ...
    async fn persists_and_recovers_task_records() -> Result<()> {
        let root = std::env::temp_dir().join(format!("wagmii-task-test-{}", Uuid::new_v4()));
        let manager =
