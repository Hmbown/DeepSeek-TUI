...
pub enum SandboxPolicy {
    ...
    ///
    /// Use this when wagmii-cli is itself running inside a container,
            ...
            SandboxPolicy::WorkspaceWrite {
                ...
                roots
                    ...
                    .map(|root| {
                        ...
                        // Protect .wagmii directories from modification
                        let wagmii_dir = root.join(".wagmii");
                        if wagmii_dir.is_dir() {
                            read_only_subpaths.push(wagmii_dir);
                        }
...
///
/// This allows fine-grained control like "allow writes to /project but not /project/.wagmii".
...
mod tests {
    ...
    fn test_writable_root_with_exceptions() {
        let root = WritableRoot::with_exceptions(
            ...
            vec![PathBuf::from("/project/.wagmii")],
        );
        assert!(root.is_path_writable(Path::new("/project/src/main.rs")));
        assert!(!root.is_path_writable(Path::new("/project/.wagmii/config")));
    }
