//! Cost estimation for Wagmii API usage.
//!
//! Pricing based on Wagmii's published rates (per million tokens).
...
fn pricing_for_model(model: &str) -> Option<ModelPricing> {
    ...
    if lower.contains("wagmii-reasoner") || lower.contains("wagmii-r1") {
        // Wagmii-R1: $0.55/M input, $2.19/M output
        Some(ModelPricing {
            input_per_million: 0.55,
            output_per_million: 2.19,
        })
    } else if lower.contains("wagmii-v3.2") {
        // Wagmii-V3.2 (with reasoning): same pricing tier as V3
        Some(ModelPricing {
            input_per_million: 0.27,
            output_per_million: 1.10,
        })
    } else if lower.contains("wagmii-chat") || lower.contains("wagmii-v3") {
        // Wagmii-V3: $0.27/M input, $1.10/M output
        Some(ModelPricing {
            input_per_million: 0.27,
            output_per_million: 1.10,
        })
    } else if lower.contains("wagmii") {
        // Generic Wagmii fallback (V3 pricing)
        Some(ModelPricing {
            input_per_million: 0.27,
            output_per_million: 1.10,
        })
