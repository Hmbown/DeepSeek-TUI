...
pub const COMMANDS: &[CommandInfo] = &[
    ...
    CommandInfo {
        ...
        description: "Show Wagmii dashboard and docs links",
        usage: "/links",
    },
    CommandInfo {
        name: "home",
    ...
    CommandInfo {
        name: "note",
        aliases: &[],
        description: "Append note to persistent notes file (.wagmii/notes.md)",
        usage: "/note <text>",
    },
    CommandInfo {
        name: "task",
...
pub fn execute(cmd: &str, app: &mut App) -> CommandResult {
    ...
    match command {
        ...
        "links" | "dashboard" | "api" => core::wagmii_links(),
        ...
        "wagmii" => CommandResult::error(
            "The /wagmii command was renamed. Use /links (aliases: /dashboard, /api).",
        ),
...
mod tests {
    ...
    #[test]
    fn command_registry_contains_config_and_links_but_not_set_or_wagmii() {
        ...
        assert!(!COMMANDS.iter().any(|cmd| cmd.name == "wagmii"));
    }
    ...
    fn execute_links_and_aliases_return_links_message() {
        ...
        for cmd in ["/links", "/dashboard", "/api"] {
            ...
            assert!(msg.contains("https://platform.wagmii.com"));
            assert!(result.action.is_none());
        }
    }

    #[test]
    fn removed_set_and_wagmii_commands_show_migration_hints() {
        let mut app = create_test_app();
        let set_result = execute("/set model wagmii-reasoner", &mut app);
        ...
        let wagmii_result = execute("/wagmii", &mut app);
        let wagmii_msg = wagmii_result
            .message
            .expect("legacy command should return an error message");
        assert!(wagmii_msg.contains("The /wagmii command was renamed"));
        assert!(wagmii_msg.contains("/links"));
        assert!(wagmii_msg.contains("/dashboard"));
        assert!(wagmii_msg.contains("/api"));
        assert!(wagmii_result.action.is_none());
    }
}
