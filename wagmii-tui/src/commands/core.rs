...
use crate::config::{COMMON_WAGMII_MODELS, canonical_model_name};
...
pub fn model(app: &mut App, model_name: Option<&str>) -> CommandResult {
    if let Some(name) = model_name {
        ...
        let supported = COMMON_WAGMII_MODELS.join(", ");
...
/// Show `Wagmii` dashboard and docs links
pub fn wagmii_links() -> CommandResult {
    CommandResult::message(
        "Wagmii Links:\n\
─────────────────────────────\n\
Dashboard: https://platform.wagmii.com\n\
Docs:      https://platform.wagmii.com/docs\n\n\
...
pub fn home_dashboard(app: &mut App) -> CommandResult {
    ...
    // Basic info
    let _ = writeln!(stats, "Wagmii CLI Home Dashboard");
    let _ = writeln!(stats, "============================================");
...
mod tests {
    ...
    fn test_help_links_topic_shows_aliases() {
        ...
        assert!(msg.contains("Show Wagmii dashboard and docs links"));
    ...
    fn test_model_change_updates_state() {
        let mut app = create_test_app();
        let old_model = app.model.clone();
        let result = model(&mut app, Some("wagmii-reasoner"));
        assert!(result.message.is_some());
        let msg = result.message.unwrap();
        assert!(msg.contains(&old_model));
        assert!(msg.contains("wagmii-reasoner"));
        assert!(matches!(
            result.action,
            Some(AppAction::UpdateCompaction(_))
        ));
        assert_eq!(app.model, "wagmii-reasoner");
    ...
    fn test_model_change_rejects_invalid_model() {
        ...
        assert!(msg.contains("wagmii-chat"));
        assert!(msg.contains("wagmii-reasoner"));
        assert!(result.action.is_none());
    }
    ...
    #[test]
    fn test_wagmii_links() {
        let result = wagmii_links();
        assert!(result.message.is_some());
        let msg = result.message.unwrap();
        assert!(msg.contains("Wagmii Links"));
    ...
    fn test_home_dashboard_includes_all_sections() {
        ...
        assert!(result.message.is_some());
        let msg = result.message.unwrap();
        assert!(msg.contains("Wagmii CLI Home Dashboard"));
