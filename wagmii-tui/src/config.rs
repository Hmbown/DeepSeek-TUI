//! Configuration loading and defaults for wagmii-cli.
...
pub const DEFAULT_TEXT_MODEL: &str = "wagmii-reasoner";
const API_KEYRING_SENTINEL: &str = "__KEYRING__";
pub const COMMON_WAGMII_MODELS: &[&str] = &["wagmii-chat", "wagmii-reasoner"];
...
pub fn canonical_model_name(model: &str) -> Option<&'static str> {
    match model.trim().to_ascii_lowercase().as_str() {
        "wagmii-chat" | "wagmii-v3" | "wagmii-v3.2" => Some("wagmii-chat"),
        "wagmii-reasoner" | "wagmii-r1" => Some("wagmii-reasoner"),
        _ => None,
    }
}
...
pub struct CapacityConfig {
    ...
    pub wagmii_v3_2_chat_prior: Option<f64>,
    pub wagmii_v3_2_reasoner_prior: Option<f64>,
...
impl Config {
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        {
            anyhow::bail!(
                "Invalid default_text_model '{model}': expected wagmii-chat or wagmii-reasoner."
            );
        }
    ...
    /// Return the `Wagmii` base URL (normalized).
    #[must_use]
    pub fn wagmii_base_url(&self) -> String {
        let base = self
            .base_url
            .clone()
            .unwrap_or_else(|| "https://api.wagmii.com".to_string());
        normalize_base_url(&base)
    }

    /// Read the `Wagmii` API key from config/environment.
    pub fn wagmii_api_key(&self) -> Result<String> {
        // First check environment variable (highest priority)
        if let Ok(key) = std::env::var("WAGMII_API_KEY")
            && !key.trim().is_empty()
        {
            return Ok(key);
        }
        ...
        anyhow::bail!(
            "Wagmii API key not found. Set it using one of these methods:\n\
             1. Set WAGMII_API_KEY environment variable (recommended)\n\
             2. Run 'wagmii login' to save to ~/.wagmii/config.toml\n\
             3. Add 'api_key = \"your-key\"' to ~/.wagmii/config.toml"
        )
    }
...
fn home_config_path() -> Option<PathBuf> {
    effective_home_dir().map(|home| home.join(".wagmii").join("config.toml"))
}

fn env_config_path() -> Option<PathBuf> {
    if let Ok(path) = std::env::var("WAGMII_CONFIG_PATH") {
...
fn default_managed_config_path() -> Option<PathBuf> {
    #[cfg(unix)]
    {
        Some(PathBuf::from("/etc/wagmii/managed_config.toml"))
    }
    #[cfg(not(unix))]
    {
        effective_home_dir().map(|home| home.join(".wagmii").join("managed_config.toml"))
    }
}
...
fn default_requirements_path() -> Option<PathBuf> {
    #[cfg(unix)]
    {
        Some(PathBuf::from("/etc/wagmii/requirements.toml"))
    }
    #[cfg(not(unix))]
    {
        effective_home_dir().map(|home| home.join(".wagmii").join("requirements.toml"))
    }
}
...
fn default_skills_dir() -> Option<PathBuf> {
    effective_home_dir().map(|home| home.join(".wagmii").join("skills"))
}

fn default_mcp_config_path() -> Option<PathBuf> {
    effective_home_dir().map(|home| home.join(".wagmii").join("mcp.json"))
}

fn default_notes_path() -> Option<PathBuf> {
    effective_home_dir().map(|home| home.join(".wagmii").join("notes.txt"))
}

fn default_memory_path() -> Option<PathBuf> {
    effective_home_dir().map(|home| home.join(".wagmii").join("memory.md"))
}

// === Environment Overrides ===

fn apply_env_overrides(config: &mut Config) {
    if let Ok(value) = std::env::var("WAGMII_API_KEY") {
        config.api_key = Some(value);
    }
    if let Ok(value) = std::env::var("WAGMII_BASE_URL") {
        config.base_url = Some(value);
    }
    if let Ok(value) = std::env::var("WAGMII_SKILLS_DIR") {
        config.skills_dir = Some(value);
    }
    if let Ok(value) = std::env::var("WAGMII_MCP_CONFIG") {
        config.mcp_config_path = Some(value);
    }
    if let Ok(value) = std::env::var("WAGMII_NOTES_PATH") {
        config.notes_path = Some(value);
    }
    if let Ok(value) = std::env::var("WAGMII_MEMORY_PATH") {
        config.memory_path = Some(value);
    }
    if let Ok(value) = std::env::var("WAGMII_ALLOW_SHELL") {
        config.allow_shell = Some(value == "1" || value.eq_ignore_ascii_case("true"));
    }
    if let Ok(value) = std::env::var("WAGMII_APPROVAL_POLICY") {
        config.approval_policy = Some(value);
    }
    if let Ok(value) = std::env::var("WAGMII_SANDBOX_MODE") {
        config.sandbox_mode = Some(value);
    }
    if let Ok(value) = std::env::var("WAGMII_MANAGED_CONFIG_PATH") {
        config.managed_config_path = Some(value);
    }
    if let Ok(value) = std::env::var("WAGMII_REQUIREMENTS_PATH") {
        config.requirements_path = Some(value);
    }
    if let Ok(value) = std::env::var("WAGMII_MAX_SUBAGENTS")
    ...
    let capacity = config.capacity.get_or_insert(CapacityConfig {
        ...
        profile_window: None,
        wagmii_v3_2_chat_prior: None,
        wagmii_v3_2_reasoner_prior: None,
        fallback_default_prior: None,
    });

    if let Ok(value) = std::env::var("WAGMII_CAPACITY_ENABLED") {
        let val = value.trim().to_ascii_lowercase();
        capacity.enabled = Some(matches!(val.as_str(), "1" | "true" | "yes" | "on"));
    }
    if let Ok(value) = std::env::var("WAGMII_CAPACITY_LOW_RISK_MAX")
        && let Ok(parsed) = value.parse::<f64>()
    {
        capacity.low_risk_max = Some(parsed);
    }
    if let Ok(value) = std::env::var("WAGMII_CAPACITY_MEDIUM_RISK_MAX")
        && let Ok(parsed) = value.parse::<f64>()
    {
        capacity.medium_risk_max = Some(parsed);
    }
    if let Ok(value) = std::env::var("WAGMII_CAPACITY_SEVERE_MIN_SLACK")
        && let Ok(parsed) = value.parse::<f64>()
    {
        capacity.severe_min_slack = Some(parsed);
    }
    if let Ok(value) = std::env::var("WAGMII_CAPACITY_SEVERE_VIOLATION_RATIO")
        && let Ok(parsed) = value.parse::<f64>()
    {
        capacity.severe_violation_ratio = Some(parsed);
    }
    if let Ok(value) = std::env::var("WAGMII_CAPACITY_REFRESH_COOLDOWN_TURNS")
        && let Ok(parsed) = value.parse::<u64>()
    {
        capacity.refresh_cooldown_turns = Some(parsed);
    }
    if let Ok(value) = std::env::var("WAGMII_CAPACITY_REPLAN_COOLDOWN_TURNS")
        && let Ok(parsed) = value.parse::<u64>()
    {
        capacity.replan_cooldown_turns = Some(parsed);
    }
    if let Ok(value) = std::env::var("WAGMII_CAPACITY_MAX_REPLAY_PER_TURN")
        && let Ok(parsed) = value.parse::<usize>()
    {
        capacity.max_replay_per_turn = Some(parsed);
    }
    if let Ok(value) = std::env::var("WAGMII_CAPACITY_MIN_TURNS_BEFORE_GUARDRAIL")
        && let Ok(parsed) = value.parse::<u64>()
    {
        capacity.min_turns_before_guardrail = Some(parsed);
    }
    if let Ok(value) = std::env::var("WAGMII_CAPACITY_PROFILE_WINDOW")
        && let Ok(parsed) = value.parse::<usize>()
    {
        capacity.profile_window = Some(parsed);
    }
    if let Ok(value) = std::env::var("WAGMII_CAPACITY_PRIOR_CHAT")
        && let Ok(parsed) = value.parse::<f64>()
    {
        capacity.wagmii_v3_2_chat_prior = Some(parsed);
    }
    if let Ok(value) = std::env::var("WAGMII_CAPACITY_PRIOR_REASONER")
        && let Ok(parsed) = value.parse::<f64>()
    {
        capacity.wagmii_v3_2_reasoner_prior = Some(parsed);
    }
    if let Ok(value) = std::env::var("WAGMII_CAPACITY_PRIOR_FALLBACK")
        && let Ok(parsed) = value.parse::<f64>()
    {
    ...
    if config.capacity.as_ref().is_some_and(|c| {
        c.enabled.is_none()
            ...
            && c.wagmii_v3_2_chat_prior.is_none()
            && c.wagmii_v3_2_reasoner_prior.is_none()
...
fn normalize_base_url(base: &str) -> String {
    ...
    let wagmii_domains = ["api.wagmii.com", "api.wagmiii.com"];
    if wagmii_domains
        .iter()
...
pub fn save_api_key(api_key: &str) -> Result<PathBuf> {
    ...
    let content = if config_path.exists() {
        ...
        format!(
            r#"# Wagmii CLI Configuration
# Get your API key from https://platform.wagmii.com
# Or set WAGMII_API_KEY environment variable

api_key = "{key_to_write}"

# Base URL (default: https://api.wagmii.com)
...
pub fn has_api_key(config: &Config) -> bool {
    ...
    if std::env::var("WAGMII_API_KEY").is_ok_and(|k| !k.trim().is_empty()) {
        return true;
    }
...
mod tests {
    ...
    struct EnvGuard {
        ...
        wagmii_config_path: Option<OsString>,
        wagmii_api_key: Option<OsString>,
    }

    impl EnvGuard {
        fn new(home: &Path) -> Self {
            let home_str = OsString::from(home.as_os_str());
            let config_path = home.join(".wagmii").join("config.toml");
            let config_str = OsString::from(config_path.as_os_str());
            let home_prev = env::var_os("HOME");
            let userprofile_prev = env::var_os("USERPROFILE");
            let wagmii_config_prev = env::var_os("WAGMII_CONFIG_PATH");
            let api_key_prev = env::var_os("WAGMII_API_KEY");
            // Safety: test-only environment mutation guarded by a global mutex.
            unsafe {
                env::set_var("HOME", &home_str);
                env::set_var("USERPROFILE", &home_str);
                env::set_var("WAGMII_CONFIG_PATH", &config_str);
                env::remove_var("WAGMII_API_KEY");
            }
            Self {
                home: home_prev,
                userprofile: userprofile_prev,
                wagmii_config_path: wagmii_config_prev,
                wagmii_api_key: api_key_prev,
            }
        }
    }
    ...
    impl Drop for EnvGuard {
        fn drop(&mut self) {
            ...
            unsafe {
                ...
                Self::restore_var("WAGMII_CONFIG_PATH", self.wagmii_config_path.take());
                Self::restore_var("WAGMII_API_KEY", self.wagmii_api_key.take());
            }
        }
    }
    ...
    fn save_api_key_writes_config() -> Result<()> {
        ...
        let expected = temp_root.join(".wagmii").join("config.toml");
    ...
    fn test_tilde_expansion_in_paths() -> Result<()> {
        ...
        let config = Config {
            skills_dir: Some("~/.wagmii/skills".to_string()),
            ..Default::default()
        };
        let expected_skills = temp_root.join(".wagmii").join("skills");
    ...
    #[test]
    fn test_load_uses_tilde_expanded_wagmii_config_path() -> Result<()> {
        let _lock = lock_test_env();
        let nanos = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .unwrap()
        ...
        let config_path = temp_root.join(".custom-wagmii").join("config.toml");
        ensure_parent_dir(&config_path)?;
        fs::write(&config_path, "api_key = \"test-key\"\n")?;

        // Safety: test-only environment mutation guarded by a global mutex.
        unsafe {
            env::set_var("WAGMII_CONFIG_PATH", "~/.custom-wagmii/config.toml");
        }
    ...
    fn test_load_falls_back_to_home_config_when_env_path_missing() -> Result<()> {
        ...
        let home_config = temp_root.join(".wagmii").join("config.toml");
    ...
    fn test_save_api_key_doesnt_match_similar_keys() -> Result<()> {
        ...
        let config_path = temp_root.join(".wagmii").join("config.toml");
        ensure_parent_dir(&config_path)?;
        fs::write(
            &config_path,
            "api_key_backup = \"old\"\napi_key = \"current\"\n",
