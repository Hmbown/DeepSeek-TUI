...
//! `~/.wagmii/automations` (or `WAGMII_AUTOMATIONS_DIR` override).
...
pub fn default_automations_dir() -> PathBuf {
    if let Ok(path) = std::env::var("WAGMII_AUTOMATIONS_DIR") {
    ...
    dirs::home_dir()
        .map(|home| home.join(".wagmii").join("automations"))
        .unwrap_or_else(|| PathBuf::from(".wagmii").join("automations"))
}
