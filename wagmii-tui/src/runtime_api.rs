//! Runtime HTTP/SSE API for local Wagmii automation.
...
pub async fn run_http_server(
    ...
    let sessions_dir = default_sessions_dir().unwrap_or_else(|_| {
        dirs::home_dir()
            .map(|h| h.join(".wagmii").join("sessions"))
            .unwrap_or_else(|| PathBuf::from(".wagmii").join("sessions"))
    });
...
mod tests {
    ...
    > {
        let root = std::env::temp_dir().join(format!("wagmii-runtime-api-{}", Uuid::new_v4()));
    ...
    async fn session_resume_thread_creates_thread_from_saved_session() -> Result<()> {
        let root = std::env::temp_dir().join(format!("wagmii-session-resume-{}", Uuid::new_v4()));
        let sessions_dir = root.join("sessions");
        fs::create_dir_all(&sessions_dir)?;
