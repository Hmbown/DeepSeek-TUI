//! CLI entry point for the `Wagmii` client.
...
#[command(
    ...
    author,
    version,
    about = "Wagmii CLI - Chat with Wagmii",
    long_about = "Unofficial CLI for the Wagmii API.\n\nJust run 'wagmii' to start chatting.\n\nNot affiliated with Wagmii Inc."
)]
struct Cli {
    /// Subcommand to run
    #[command(subcommand)]
...
enum Commands {
    ...
    Init,
    /// Save a Wagmii API key to the config file
    Login {
...
fn run_setup(config: &Config, workspace: &Path, args: SetupArgs) -> Result<()> {
    ...
    let (aqua_r, aqua_g, aqua_b) = palette::WAGMII_SKY_RGB;
    let (sky_r, sky_g, sky_b) = palette::WAGMII_SKY_RGB;
    ...
    println!(
        "{}",
        "Wagmii Setup".truecolor(aqua_r, aqua_g, aqua_b).bold()
    );
    ...
    if run_mcp {
        ...
        println!("    Next: edit the file, then run `wagmii mcp list` or `wagmii mcp tools`.");
    }
...
async fn run_doctor(config: &Config, workspace: &Path, config_path_override: Option<&Path>) {
    ...
    let (blue_r, blue_g, blue_b) = palette::WAGMII_BLUE_RGB;
    let (sky_r, sky_g, sky_b) = palette::WAGMII_SKY_RGB;
    let (aqua_r, aqua_g, aqua_b) = palette::WAGMII_SKY_RGB;
    let (red_r, red_g, red_b) = palette::WAGMII_RED_RGB;
    ...
    println!("  wagmii-cli: {}", env!("CARGO_PKG_VERSION"));
    ...
    let default_config_dir =
        dirs::home_dir().map_or_else(|| PathBuf::from(".wagmii"), |h| h.join(".wagmii"));
    let config_path = config_path_override
        .map(PathBuf::from)
        .or_else(|| {
            std::env::var("WAGMII_CONFIG_PATH")
                .ok()
                .map(PathBuf::from)
        })
    ...
    let has_api_key = if std::env::var("WAGMII_API_KEY")
        .ok()
        .filter(|k| !k.trim().is_empty())
        .is_some()
    {
        println!(
            "  {} WAGMII_API_KEY is set",
            "✓".truecolor(aqua_r, aqua_g, aqua_b)
        );
        true
    } else if config.wagmii_api_key().is_ok() {
        println!(
            "  {} Wagmii API key found in effective config",
            "✓".truecolor(aqua_r, aqua_g, aqua_b)
        );
        true
    } else {
        println!(
            "  {} Wagmii API key not configured",
            "✗".truecolor(red_r, red_g, red_b)
        );
        println!("    Run 'wagmii' to configure interactively, or set WAGMII_API_KEY");
        false
    };
    ...
    if has_api_key {
        print!("  {} Testing connection to Wagmii API...", "·".dimmed());
        use std::io::Write;
        ...
        match test_api_connectivity().await {
            ...
            Err(e) => {
                ...
                if error_msg.contains("401") || error_msg.contains("Unauthorized") {
                    println!("    Invalid API key. Check your WAGMII_API_KEY or config.toml");
                } else if error_msg.contains("403") || error_msg.contains("Forbidden") {
                    println!(
                        "    API key lacks permissions. Verify key is active at platform.wagmii.com"
                    );
    ...
    if mcp_config_path.exists() {
        ...
        println!("    Run `wagmii mcp init` or `wagmii setup --mcp`.");
    }
    ...
    if !agents_skills_dir.exists() && !local_skills_dir.exists() && !global_skills_dir.exists() {
        println!("    Run `wagmii setup --skills` (or add --local for ./skills).");
    }
...
async fn run_models(config: &Config, args: ModelsArgs) -> Result<()> {
    ...
    let client = WagmiiClient::new(config)?;
...
fn list_sessions(limit: usize, search: Option<String>) -> Result<()> {
    ...
    if sessions.is_empty() {
        ...
        println!(
            ...
            "wagmii".truecolor(blue_r, blue_g, blue_b)
        );
        return Ok(());
    }
    ...
    println!(
        "Resume with: {} {}",
        "wagmii --resume".truecolor(blue_r, blue_g, blue_b),
        "<session-id>".dimmed()
    );
    println!(
        "Continue latest: {}",
        "wagmii --continue".truecolor(blue_r, blue_g, blue_b)
    );
...
fn init_project() -> Result<()> {
    ...
    match create_default_agents_md(&workspace) {
        Ok(path) => {
            ...
            println!("The instructions will be loaded automatically when you run wagmii.");
        }
        Err(e) => {
            println!(
...
fn load_config_from_cli(cli: &Cli) -> Result<Config> {
    let profile = cli
        .profile
        .clone()
        .or_else(|| std::env::var("WAGMII_PROFILE").ok());
...
async fn run_mcp_command(config: &Config, command: McpCommand) -> Result<()> {
    ...
    match command {
        McpCommand::Init { force } => {
            ...
            println!("Edit the file, then run `wagmii mcp list` or `wagmii mcp tools`.");
            Ok(())
        }
        McpCommand::List => {
