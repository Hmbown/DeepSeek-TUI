...
/// Resolve `~/.wagmii/memory`.
#[must_use]
pub fn default_capacity_memory_dir() -> PathBuf {
    capacity_memory_dirs()
        .into_iter()
        .next()
        .unwrap_or_else(|| PathBuf::from(".").join(".wagmii").join("memory"))
}

fn capacity_memory_dirs() -> Vec<PathBuf> {
    if let Ok(raw) = std::env::var("WAGMII_CAPACITY_MEMORY_DIR") {
    ...
    if let Some(home) = dirs::home_dir() {
        dirs.push(home.join(".wagmii").join("memory"));
    }
