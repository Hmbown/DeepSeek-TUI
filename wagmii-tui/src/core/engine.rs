...
pub struct Engine {
    config: EngineConfig,
    wagmii_client: Option<WagmiiClient>,
    wagmii_client_error: Option<String>,
    session: Session,
...
fn emit_tool_audit(event: serde_json::Value) {
    let Some(path) = std::env::var_os("WAGMII_TOOL_AUDIT_LOG") else {
        return;
    };
...
impl Engine {
    ...
    pub fn new(config: EngineConfig, api_config: &Config) -> (Self, EngineHandle) {
        ...
        let (wagmii_client, wagmii_client_error) = match WagmiiClient::new(api_config) {
        ...
        while let Some(op) = self.rx_op.recv().await {
            match op {
                ...
                Op::SpawnSubAgent { prompt } => {
                    let Some(client) = self.wagmii_client.clone() else {
                        let message = self
                            .wagmii_client_error
                            .as_deref()
    ...
    async fn handle_send_message(
        ...
        builder = builder
            .with_review_tool(self.wagmii_client.clone(), self.session.model.clone())
        ...
        let tool_registry = match mode {
            AppMode::Agent | AppMode::Yolo => {
                if self.config.features.enabled(Feature::Subagents) {
                    let runtime = if let Some(client) = self.wagmii_client.clone() {
        ...
        let (status, error) = self
            .handle_wagmii_turn(&mut turn, tool_registry.as_ref(), tools, mode)
            .await;
    ...
    /// Handle a turn using the Wagmii API.
    #[allow(clippy::too_many_lines)]
    async fn handle_wagmii_turn(
        &mut self,
        ...
        let client = self
            .wagmii_client
            .clone()
            .expect("Wagmii client should be configured");
        ...
        loop {
            ...
            // Wagmii chat API rejects assistant messages that contain only
