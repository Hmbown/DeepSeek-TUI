...
//!
//! This module keeps Wagmii-only execution while exposing Codex-like lifecycle
...
impl RuntimeThreadManagerConfig {
    ...
    pub fn from_task_data_dir(task_data_dir: PathBuf) -> Self {
        let data_dir = if let Ok(override_dir) = std::env::var("WAGMII_RUNTIME_DIR") {
...
mod tests {
    ...
    fn test_runtime_dir() -> PathBuf {
        std::env::temp_dir().join(format!("wagmii-runtime-threads-{}", Uuid::new_v4()))
    }
