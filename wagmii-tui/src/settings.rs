...
//!
//! Settings are stored at ~/.config/wagmii/settings.toml
...
impl Settings {
        ...
        match key {
            ...
            "default_model" | "model" => {
                ...
                let Some(model) = canonical_model_name(trimmed) else {
                    anyhow::bail!(
                        "Failed to update setting: invalid model '{value}'. Expected: wagmii-chat, wagmii-reasoner, or none/default."
                    );
                };
    ...
    pub fn available_settings() -> Vec<(&'static str, &'static str)> {
        vec![
            ...
            (
                "default_model",
                "Default model: wagmii-chat or wagmii-reasoner",
            ),
        ]
    }
}
