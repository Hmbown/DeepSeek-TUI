...
#   TOKEN     – PAT with contents:write on TAP_REPO

set -euo pipefail

: "${TOKEN:?Set HOMEBREW_TAP_PAT or RELEASE_TAG_PAT}"
: "${TAG:?}"
: "${MANIFEST:?}"
: "${TAP_REPO:?}"
...
readonly FORMULA_FILE="/tmp/deepseek-tui.rb"
...
git clone --depth 1 "${TAP_URL}" /tmp/homebrew-tap

cp "${FORMULA_FILE}" /tmp/homebrew-tap/Formula/deepseek-tui.rb
