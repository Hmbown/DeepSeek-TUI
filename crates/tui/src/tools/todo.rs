        ...
        for item in todos {
            if item.get("id").is_some() {
                return Err(ToolError::invalid_input(
                    "Todo item 'id' is generated by checklist_write; omit it and use checklist_update to modify existing items by id",
                ));
            }
            ...
            parsed_todos.push((content.to_string(), status));
        }

        let mut list = self.todo_list.lock().await;

        // Clear and rebuild the list only after the full replacement payload
        // validates, so malformed model output cannot wipe existing progress.
        list.clear();

        for (content, status) in parsed_todos {
...
mod tests {
    ...
    #[test]
    fn checklist_write_schema_disallows_model_supplied_ids() {
        let tool = TodoWriteTool::checklist(new_shared_todo_list());
        ...
        assert!(schema["properties"]["todos"]["items"]["properties"]["id"].is_null());
    }

    #[tokio::test]
    async fn checklist_write_rejects_model_supplied_ids() {
        let todos = new_shared_todo_list();
        let tool = TodoWriteTool::checklist(todos.clone());
        let context = ToolContext::new(std::env::temp_dir());
        tool.execute(
            json!({
                "todos": [
                    { "content": "keep existing progress", "status": "in_progress" }
                ]
            }),
            &context,
        )
        .await
        .expect("initial checklist write succeeds");

        let err = tool
            .execute(
                json!({
                    "todos": [
                        { "id": 9, "content": "do not preserve ids", "status": "pending" }
                    ]
                }),
                &context,
            )
            .await
            .expect_err("checklist_write should reject caller supplied ids");

        assert!(err.to_string().contains("id"));
        assert!(err.to_string().contains("checklist_update"));

        let snapshot = todos.lock().await.snapshot();
        assert_eq!(snapshot.items.len(), 1);
        assert_eq!(snapshot.items[0].content, "keep existing progress");
