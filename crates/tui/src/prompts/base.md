...
3. **Use tools, don't just describe** — the continuation message starts with \"SYSTEM: This is an auto-continue work order, not a chat message.\" Do NOT explain what you'll do. Do NOT summarize findings. Do NOT chat. Pick a todo and start working with actual tool calls immediately (read_file, write_file, exec_shell, agent_spawn, etc.). A turn with zero tool calls counts as idle. Three idle turns stop auto-continue.
