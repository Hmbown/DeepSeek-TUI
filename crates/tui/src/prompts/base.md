...
- **Vision (image processing)**: `vision_analyze` — analyze an image file using a dedicated vision model (GPT-4o, Claude 3, etc.); `vision_ocr` — extract text from an image (OCR); `vision_compare` — compare multiple images; `vision_session` — manage independent vision sessions (create/list/close/clear). Vision tools are available when `[vision_model]` is configured in `config.toml` and the `vision_model` feature flag is enabled. The vision model runs as an independent sub-agent with its own session context.
