...
- **Vision (image processing)**: `vision_analyze` — analyze an image file using a dedicated vision model (e.g., gemini-3.1-flash-lite-preview); `vision_ocr` — extract text from an image (OCR). Vision tools are available when `[vision_model]` is configured in `config.toml` and the `vision_model` feature flag is enabled. The vision model runs as an independent sub-agent with its own session context.
