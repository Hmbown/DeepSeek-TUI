...
- **Sub-agents**: `agent_spawn` (`spawn_agent`, `delegate_to_agent`), `agent_result`, `agent_cancel` (`close_agent`), `agent_list`, `agent_wait` (`wait`), `agent_send_input` (`send_input`), `agent_assign` (`assign_agent`), `resume_agent`. Use `profile` parameter to activate agency-agent personas (frontend-developer, security-engineer, etc.).
- **Agent profiles**: `ProfileRegistry` supports import of [agency-agents](https://github.com/msitarzewski/agency-agents) — 144+ specialized AI personas across 12 divisions. Import via `git clone` to `~/.deepseek/agency-agents/`. Profile names are slugified: "Frontend Developer" → `frontend-developer`.
- **Multi-agent orchestration**: `goap_plan` (A* goal-oriented action planning), `swarm_init` (multi-agent topology coordinator), `manager_delegate` (CrewAI-style hierarchical delegation — create → assign → dispatch → record → synthesize).
- **Agent memory**: `memory_store` (store facts with key/type/tags), `memory_search` (search across short-term/long-term/entity memory).
- **Agent graph (LangGraph-style)**: `graph_execute` — define and execute state-machine workflows with conditional routing, checkpoints, and streaming events.
- **Headless browser (obscura)**: `obscura` — fast stealth headless browser for JS-rendered page extraction, parallel multi-URL scraping, CDP server. 30 MB/instant start. Actions: fetch, scrape, serve. Built-in anti-fingerprinting + tracker blocking.
- **Browser automation (agent-browser)**: `browser` — control a real Chrome browser via agent-browser CLI. Actions: navigate, snapshot (accessibility tree with @eN refs), click, fill, type_text, screenshot, get_text, wait, evaluate (JS), close. For advanced commands (tabs, network, auth), use `exec_shell agent-browser ...`.
- **Web pipeline**: `web_pipeline` — deep crawl (BFS/DFS/best-first), BM25 content filtering, and smart chunking for LLM processing.
