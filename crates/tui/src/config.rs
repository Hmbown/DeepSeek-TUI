...
use deepseek_execpolicy::{ExecPolicyEngine, Ruleset, ToolPermissionRule};
...
fn merge_permissions_config(
    ...
    if override_cfg.rules.is_empty() {
        base
    } else {
        override_cfg
    }
...
"#,
        ...
        let decision = config.exec_policy_engine().check_tool_permission(
            deepseek_execpolicy::ToolPermissionContext {
                tool: "read_file",
                command: None,
        ...
        assert_eq!(
            decision.decision,
            Some(deepseek_execpolicy::PermissionDecision::Ask)
        );
        Ok(())
    }
