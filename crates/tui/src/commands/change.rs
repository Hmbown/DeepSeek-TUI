...
pub fn change(app: &mut App, version: Option<&str>) -> CommandResult {
    ...
    let latest_section = match section {
        ...
        None => {
            let msg = if let Some(ver) = version {
                format!("Could not find version \"{ver}\" in the bundled DeepSeek-TUI changelog.")
            } else {
    ...
    let prev_ver = previous_version_hint(DEEPSEEK_TUI_CHANGELOG, version);
    let prev_hint = if let Some(ref v) = prev_ver {
        let template = tr(locale, MessageId::CmdChangePreviousVersion);
        format!("\n\n{}", template.replace("{version}", v))
    } else {
        String::new()
    };
...
{section_text}{prev_hint}"
        ...
        let prev_hint_en = if let Some(ref v) = prev_ver {
            let template = tr(Locale::En, MessageId::CmdChangePreviousVersion);
            format!("\n\n{}", template.replace("{version}", v))
        } else {
            String::new()
        };
        let translation_source = format!("{latest_section}{prev_hint_en}");
...
fn extract_latest_changelog_section(content: &str) -> Option<String> {
    ...
    let first_idx = lines.iter().position(|l| is_changelog_heading(l))?;
    ...
    loop {
        let end = lines
            ...
            .enumerate()
            .skip(pos + 1)
            .find(|(_, line)| is_changelog_heading(line))
...
fn extract_changelog_section_by_version(content: &str, version: &str) -> Option<String> {
    ...
    let start = lines
        .iter()
        .position(|line| changelog_heading_version(line) == Some(version))?;
...
fn extract_previous_version_number(content: &str) -> Option<String> {
    ...
    loop {
        let end = lines
            ...
            .enumerate()
            .skip(pos + 1)
            .find(|(_, l)| is_changelog_heading(l))
            .map_or(lines.len(), |(i, _)| i);
...
fn is_changelog_heading(line: &str) -> bool {
    changelog_heading_version(line).is_some()
}

fn changelog_heading_version(line: &str) -> Option<&str> {
    line.trim()
        .strip_prefix("## [")
...
fn next_contentful_version_after(lines: &[&str], mut pos: usize) -> Option<String> {
    while pos < lines.len() {
        ...
        let Some(heading_ver) = changelog_heading_version(heading) else {
            pos += 1;
            continue;
        };
        ...
        if section_has_body_content(&lines[pos + 1..end]) {
            return Some(heading_ver.to_string());
        }
...
Content.\n";
    ...
    #[test]
    fn malformed_version_headings_do_not_panic() {
        let content = "\n\
## [\n\
\n\
Malformed heading content.\n\
\n\
## [0.8.1] - 2026-01-01\n\
\n\
Release content.\n";
        let section =
            extract_changelog_section_by_version(content, "0.8.1").expect("should find 0.8.1");
        assert!(section.contains("Release content"));
        assert!(next_contentful_version_after(&["## ["], 0).is_none());
    }
