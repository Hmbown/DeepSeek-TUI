...
impl ExecPolicyEngine {
    ...
    /// Prefixes are collected in layer order (BuiltinDefault → Agent → User); `find()`
    /// returns the first match, so earlier layers can shadow later ones for trust rules.
    /// Deny rules are always checked before trust rules in `check()`, so deny wins
    /// regardless of which layer introduced it — no explicit priority shadowing needed.
