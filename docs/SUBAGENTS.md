...
agent spawns one with a focused task, receives an `agent_id`
...
This doc covers the role taxonomy. For the tool surface
(`agent_spawn` / `agent_wait` / `agent_result` / `agent_cancel` /
`agent_list` / `agent_send_input` / `agent_resume` / `agent_assign`),
see `prompts/base.md` "Sub-Agent Strategy" and the inline tool
descriptions.

## Role taxonomy

The `agent_type` field on `agent_spawn` selects a system-prompt
posture for the child. Each role represents a distinct approach
to the work — not just a different label.
...
- **`explore`** — when the parent needs evidence before deciding next steps. Explorers are fast and cheap;
  spawn 2–3 in parallel for independent regions.
- **`plan`** — when the parent has an objective but no executable
  decomposition. Planners write artifacts (`update_plan` rows,
  `checklist_write` entries) but do not carry them out.
- **`review`** — when there is already a change and the parent wants
  it graded. Reviewers do not patch — they describe the fix in the
...
- **`verifier`** — when the parent needs an authoritative pass/fail
  on the test suite or other validation. Verifiers do not fix
  failures; they capture the failing assertion and stack and put
