...
1. If a foreground shell command is running, press `Ctrl+B` to choose whether to background it or cancel the current turn.
...
5. On restart, verify the previous queued/in-flight runtime turn is marked as interrupted, not left in a running state.
