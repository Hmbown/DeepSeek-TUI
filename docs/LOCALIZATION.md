...
This document covers UI localization in the TUI. It does not affect model output language, provider behavior, or API payload support. Media attachments remain local path text references unless native media payload support is added separately.
...
`MCP`, `CLI`, `TUI`, and key combinations such as `Enter`, `Esc`, `Tab`, `PgUp`, and
`PgDn`.
